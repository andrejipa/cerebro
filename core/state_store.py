...
class StateStore:
    ...
    def __init__(self, root: str | Path):
        ...
        self._runtime_lock_service = StateRuntimeLockService(
            cerebro_dir=self.cerebro_dir,
            lock_path=self.lock_path,
            error_cls=StateStoreError,
            timeout_seconds=lambda: RUNTIME_LOCK_TIMEOUT_SECONDS,
            poll_seconds=lambda: RUNTIME_LOCK_POLL_SECONDS,
            release_retry_limit=lambda: RUNTIME_LOCK_RELEASE_RETRY_LIMIT,
            pid_probe=lambda pid: os.kill(pid, 0),
            monotonic=lambda: time.monotonic(),
            sleep_fn=lambda seconds: time.sleep(seconds),
            get_pid=lambda: os.getpid(),
        )
        self._retention = StateRetentionService(
            cerebro_dir=self.cerebro_dir,
            events_path=self.events_path,
            artifacts_dir=self.artifacts_dir,
            trash_dir=self.trash_dir,
            error_cls=StateStoreError,
            parse_parallel_approach_consolidation_line=self._parse_parallel_approach_consolidation_line,
            parse_event_log_event_type=self._parse_event_log_event_type,
            iter_live_runtime_artifact_refs=self._iter_live_runtime_artifact_refs,
            resolve_runtime_artifact_ref=self._resolve_runtime_artifact_ref,
            write_json_atomic=self._write_json_atomic,
            commit_trace_events=self._commit_trace_events,
        )
    ...
    def describe_retention_policy(self) -> dict:
        ...
        return self._retention.describe_retention_policy()
    ...
    def _load_pending_retention_archive(self) -> tuple[Path, dict] | None:
        ...
        return self._retention.load_pending_retention_archive()
    ...
    def _build_retention_pending_manifest(
        ...
        *,
        ...
        return self._retention.build_retention_pending_manifest(
            archive_root_ref=archive_root_ref,
    ...
    def _build_retention_archive_manifest(
        ...
        *,
        ...
        return self._retention.build_retention_archive_manifest(
            created_at=created_at,
            policy=policy,
            archived_event_lines=archived_event_lines,
            archived_event_bytes=archived_event_bytes,
    ...
    def _build_retention_apply_result(
        ...
        *,
        ...
        return self._retention.build_retention_apply_result(
            policy=policy,
            archived_event_lines=archived_event_lines,
            archived_event_bytes=archived_event_bytes,
            archived_group_count=archived_group_count,
    ...
    def _event_log_contains_retention_event(self, event_id: str) -> bool:
        ...
        return self._retention.event_log_contains_retention_event(event_id)
    ...
    def _artifact_retention_group_path(self, relative_artifact_path: str) -> str | None:
        ...
        return self._retention.artifact_retention_group_path(relative_artifact_path)

    def _remove_empty_artifact_parents(self, start: Path) -> None:
        """Collapse empty artifact parents after archiving one group."""
        self._retention.remove_empty_artifact_parents(start)
