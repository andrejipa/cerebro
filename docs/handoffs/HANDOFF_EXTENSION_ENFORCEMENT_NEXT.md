...
- Stop point: current tests now cover package drift, extension READMEs, shared runtime-path rejection, main dynamic evasions, tracked-file allowlisting, and Git-mode checks for symlink/executable entries in `extensions/`
...
- Safe next action after release:
  - keep hardening outside the runtime and stop until a concrete new evasion path appears beyond the current tracked-file and Git-metadata checks
