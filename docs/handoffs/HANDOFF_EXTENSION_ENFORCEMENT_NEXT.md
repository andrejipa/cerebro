...
- Stop point: current tests now cover package drift, extension READMEs, shared runtime-path rejection, main dynamic evasions, direct filesystem-read bans in extensions, process-spawning bans, tracked-file allowlisting, Git-mode checks for symlink/executable entries in `extensions/`, and canonical CLI command names without human aliases
