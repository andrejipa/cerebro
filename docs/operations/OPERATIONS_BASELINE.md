...
- `validate_state()` retries up to `VALIDATION_RETRY_LIMIT = 3` times when a concurrent writer changes `state.json` mid-validation (detected as a revision mismatch on the atomic save); if state still changes after 3 attempts it returns `state_changed_during_validation` rather than an internal error — callers should rerun the operation
